import { Component, Input } from '@angular/core';
import { EvolucionService } from '../../../../../services/evolucion.service';
import { FormGroup, FormBuilder, Validators } from '@angular/forms';
import { LoginService } from '../../../../../../login/services/login.service';
import { ConfirmationService, MessageService, Message } from 'primeng/api';
import { jsPDF } from "jspdf";
import { Router, ActivatedRoute } from '@angular/router';
const pdfMake = require("pdfmake/build/pdfmake");
const pdfFonts = require("pdfmake/build/vfs_fonts");


@Component({
  selector: 'app-paso5', 
  templateUrl: './paso5.component.html',
  styleUrls: ['./paso5.component.css']
})
export class Paso5Component {


  get id_medico() {
    return this.lsv.Getid_usuario;
  }

  confirmarEvolucion: boolean = true;

  diagnostico: string = '';
  plan       : string = '';
  evolucion  : string = '';
  fecha_alta : string = '';
  estudios   : string = ''; 
  
  datos_paciente: any[] = [];
  datos_medico: any[] = [];
  datos_servicio:any[] = [];
  id_evolucion: number = 0;
  mensaje!: Message[];

  fecha_ingreso:string = "";

  agregarEvolucionForm: FormGroup = this.fb.group({
    fechaAlta             : ['', Validators.required],
    servicio              : ['', Validators.required],
    diagnostico           : ['', Validators.required],
    plan                  : ['', Validators.required],
    evolucion             : ['', Validators.required],
    estudio               : ['', Validators.required],
    participacion_interno : [false],
    nombre_interno        : ['']
  });

  servicios: any[] = [];

  constructor(private esv: EvolucionService,
              private fb: FormBuilder,
              private lsv: LoginService,
              private cs: ConfirmationService,
              private msg: MessageService,
              private router: Router,
              private rutaActiva: ActivatedRoute) {
    const { id } = rutaActiva.snapshot.params;
    this.id_evolucion = id;
    const { diagnostico, plan, fecha_alta, evolucion, estudio_complementario } = esv._dataEvolucion[0];

    this.diagnostico =  diagnostico;
    this.plan        = plan;
    this.evolucion   = evolucion;
    this.fecha_alta  = fecha_alta;
    this.estudios    = estudio_complementario;

    // this.esv.obtenerTodosLosServicios().subscribe(response => {
    //   this.servicios = response;
    // });

    // esv.obtenerEvolucionConfirmar(id).subscribe((resp: any) => {
    //   if(resp.length === 0){ 
    //     return
    //   }

    //   if(resp[0].id_estado === 1){
    //     this.agregarEvolucionForm.patchValue({
    //       servicio: resp[0].id_servicio
    //     })
    //     if(resp[0].interno_medicina != 'no'){
    //       this.agregarEvolucionForm.patchValue({
    //         participacion_interno: true,
    //         nombre_interno: resp[0].interno_medicina
    //       })
    //     }
    //     this.confirmarEvolucion = false;
    //     return;
    //   }
     
    // })

    if(fecha_alta === undefined) {
      this.agregarEvolucionForm.patchValue({
        diagnostico: this.diagnostico,
        plan       : this.plan,
        evolucion  : this.evolucion,
        estudio    : this.estudios
      });
      return;
    }

    this.agregarEvolucionForm.patchValue({
      fechaAlta  : this.fecha_alta.substring(0, 10),
      diagnostico: this.diagnostico,
      plan       : this.plan,
      evolucion  : this.evolucion,
      estudio    : this.estudios
    });

  }

  agregarEvolucion () {

    if(this.agregarEvolucionForm.valid) {

      const { servicio, fechaAlta, participacion_interno, nombre_interno } = this.agregarEvolucionForm.value;

      this.esv._dataEvolucion[0].id_medico = this.id_medico;
      this.esv._dataEvolucion[0].fecha_alta = fechaAlta;
      this.esv._dataEvolucion[0].id_servicio = servicio;
      this.esv._dataEvolucion[0].nombre_interno = nombre_interno;
      this.esv._dataEvolucion[0].participacion_interno = participacion_interno;

      this.cs.confirm({
        message: '¿Esta seguro de realizar esta evolucion?',
        accept : () => {
          // this.esv.agregarEvolucionPaciente().subscribe(async (resp) => {
          //   this.msg.add({severity: "success", summary: "Exito!", detail: "La evolucion fue realizada correctamente"})

          //   let Paciente: any = await this.obtenerDatosPaciente();
          //   let Medico: any = await this.obtenerDatosMedico();
          //   let Servicio:any = await this.obtenerDatosServicio();
            
          //   //limpieza de objeto con datos de la evolucion
          //   this.datos_paciente = Paciente;
          //   this.datos_medico = Medico;
          //   this.datos_servicio = Servicio;
          //   this.esv._dataEvolucion[0].diagnostico = '';
          //   this.esv._dataEvolucion[0].estudio_complementario = '';
          //   this.esv._dataEvolucion[0].evolucion = '';
          //   this.esv._dataEvolucion[0].fecha_alta = '';
          //   this.esv._dataEvolucion[0].fecha_registro = '';
          //   this.esv._dataEvolucion[0].id_medico = 0;
          //   this.esv._dataEvolucion[0].id_servicio = 0;
          //   this.esv._dataEvolucion[0].plan = '';
          //   this.esv._dataEvolucion[0].nombre_interno = '';
          //   this.esv._dataEvolucion[0].participacion_interno = false;
          //   this.pdfNuevo();
          //   setTimeout(() => {
          //     const { id_paciente } = this.esv._dataEvolucion[0];
          //     this.router.navigate(['/inicio/pacientes/evoluciones', id_paciente]);
          //   }, 1000);
          // });
        }
      });
    }else{
      this.msg.add({severity: "error",summary:"Por favor completar campos para poder continuar.", key: "error"});
      setTimeout(() => {
        this.mensaje = [];
      }, 2000);
    }

  }
  
  get idPaciente(){
    return this.esv.getIdPaciente;
  }
  
  obtenerDatosPaciente = () => {
    return new Promise((resolve, reject) =>{
      try {
        //obtener datos del paciente
        // this.esv.obenerInformacionDelPaciente(this.idPaciente).subscribe(response =>{
        //   resolve(response);
        // });      
      } catch (error) {
        reject(error);
      }
    })
  }
  
  

  obtenerDatosMedico = () => {
    return new Promise((resolve, reject) =>{
      try {
        //obtener datos del medico
        // this.esv.obtenerDatosMedicos(this.esv._dataEvolucion[0].id_medico).subscribe(response =>{
        //   resolve(response);
        // });
      } catch (error) {
          reject(error);
      }
    })
  }

  obtenerDatosServicio = () => {
    return new Promise((resolve, reject) =>{
      try {
        //obtener datos del servicio
        // this.esv.obtenerDatosServicio(this.agregarEvolucionForm.controls["servicio"].value).subscribe(response =>{
        //   resolve(response);
        // });      
      } catch (error) {
          reject(error);
      };
    });
  };

  agregarEvolucionEnProceso() {
    if(this.agregarEvolucionForm.valid) {

      const { servicio, fechaAlta, nombre_interno, participacion_interno } = this.agregarEvolucionForm.value;
      this.esv._dataEvolucion[0].id_medico = this.id_medico;
      this.esv._dataEvolucion[0].fecha_alta = fechaAlta;
      this.esv._dataEvolucion[0].id_servicio = servicio;
      this.esv._dataEvolucion[0].nombre_interno = nombre_interno;
      this.esv._dataEvolucion[0].participacion_interno = participacion_interno;

      this.cs.confirm({
        message: '¿Esta seguro de guardar esta evolucion?',
        accept : ()=> {
          // this.esv.agregarEvolucionPacienteEnProceso().subscribe(resp => {
          //   this.msg.add({severity: "success", summary: "Exito!", detail: "La evolucion fue realizada correctamente"})
          //   setTimeout(() => {
          //     const { id_paciente } = this.esv._dataEvolucion[0];
          //     this.router.navigate(['/inicio/pacientes/evoluciones', id_paciente]);
          //   }, 1000);
          // });
        } 
      });
    } else{
      this.msg.add({severity: "error",summary:"Por favor completar campos para poder continuar.", key: "error"});
      setTimeout(() => {
        this.mensaje = [];
      }, 2000);
    }
  };


  confirmarEvolucionEnProceso(){
    if(this.agregarEvolucionForm.valid){
      
      const { fechaAlta, servicio, nombre_interno, participacion_interno } = this.agregarEvolucionForm.value;
      this.esv._dataEvolucion[0].fecha_alta = fechaAlta;
      this.esv._dataEvolucion[0].id_servicio = servicio;

      participacion_interno === true ? this.esv._dataEvolucion[0].nombre_interno = nombre_interno : this.esv._dataEvolucion[0].nombre_interno = 'no';
      this.cs.confirm({
        message: '¿Esta seguro de confirmar esta evolucion?',
        accept : ()=> {
          // this.esv.confirmarEvolucion(this.id_evolucion).subscribe(async(resp) => {
          //   this.msg.add({severity: "success", summary: "Exito!", detail: "La evolucion fue realizada correctamente"})

          //   let Paciente: any = await this.obtenerDatosPaciente();
          //   let Medico: any = await this.obtenerDatosMedico();
          //   let Servicio:any = await this.obtenerDatosServicio();
            
          //   this.datos_paciente = Paciente;
          //   this.datos_medico = Medico;
          //   this.datos_servicio = Servicio;

          //   this.pdfNuevo();
          //   setTimeout(() => {
          //     this.router.navigate(['/inicio/pacientes/evoluciones', this.idPaciente]);
          //   }, 1000);
          // });
        }
      });
    }
  };

  //formatea la fecha de nacimiento del paciente y fecha de alta del paciente
  formatearFecha(fechaString: string) {
    const fechaObjeto: Date = new Date(fechaString);

    const dia: number = fechaObjeto.getDate() +1;
    const mes: number = fechaObjeto.getMonth() + 1;
    const anio: number = fechaObjeto.getFullYear();
  
    const mesFormateado: string = mes.toString().padStart(2, "0");
    const fechaStringNueva: string = `${dia}-${mesFormateado}-${anio}`;
  
    return fechaStringNueva;
  }

  // funciona para la fecha de ingreso del paciente
  transformarFecha(fecha: string): string {
    const partes = fecha.split('-'); // Dividir la fecha en partes: año, mes, día
    const dia = partes[2];
    const mes = partes[1];
    const anio = partes[0];
    const fechaTransformada = `${dia}-${mes}-${anio}`;
    return fechaTransformada;
  }

pdfNuevo = () => {

    //DATOS CABECERA
    let nombreCompletoPaciente = `${this.datos_paciente[0].nombres} ${this.datos_paciente[0].apellidos}`;
    let run_paciente = this.datos_paciente[0].run;

    //se usa para calcular la edad del paciente
    let fechaNacimiento =  new Date(this.datos_paciente[0].fecha_nacimiento);
    const fechaNacimientoParaPdf = this.datos_paciente[0].fecha_nacimiento.slice(0, 'yyyy-mm-dd'.length).split('-').reverse().join('-');

    //se usa solo para verlo en el formato del pdf
    let fecha_ingreso = this.datos_paciente[0].fecha_ingreso.slice(0, 'yyyy-mm-dd'.length).split('-').reverse().join('-');


    let fecha_hoy = new Date().toLocaleDateString();
    //datos del medico
    let run_medico = this.datos_medico[0].run;
    let nombreCompletoMedico = `${this.datos_medico[0].nombres} ${this.datos_medico[0].apellidos}`;

    let fecha_alta = this.agregarEvolucionForm.controls["fechaAlta"].value.slice(0, 'yyyy-mm-dd'.length).split('-').reverse().join('-');

    let servicio = this.datos_servicio[0].nombre;

    //calcular la edad del paciente....
    let hoy = new Date();
    let edad = hoy.getFullYear() - fechaNacimiento.getFullYear();
    let diferenciaMeses = hoy.getMonth() - fechaNacimiento.getMonth();
    if(diferenciaMeses < 0 ||  ( diferenciaMeses === 0 && hoy.getDate() < fechaNacimiento.getDate() )){
      edad --;
    }; 
    //DATOS DE EVOLUCION MEDICA
    let diagnostico = this.agregarEvolucionForm.controls["diagnostico"].value;
    let evolucion = this.agregarEvolucionForm.controls["evolucion"].value;
    let plan = this.agregarEvolucionForm.controls["plan"].value;
    let estudios = this.agregarEvolucionForm.controls["estudio"].value;
    let interno_medicina = this.agregarEvolucionForm.controls["nombre_interno"].value;
    let participacion_interno = this.agregarEvolucionForm.controls["participacion_interno"].value;
    

    !participacion_interno ? interno_medicina = '' : interno_medicina = interno_medicina;

    let imagen: string = 'data:image/png;base64,'
    
    interno_medicina === '' ? interno_medicina = 'NO' : interno_medicina = interno_medicina;
    var documento_pdf = {
      info: {
        title: `EVOLUCIÓN DE ${nombreCompletoPaciente} ${fecha_hoy}`,
        author: 'Darwin'
      },
      pageMargins: [40, 180, 40, 60],
      pageSize: 'LETTER',

      header: [
          { image: imagen, width: 150, margin: [430, 10, 0, 10] },
          {text: `\nEVOLUCIÓN DEL PACIENTE: ${fecha_hoy}`, style:['titulo']},
          {text: `\n\nNOMBRE: ${nombreCompletoPaciente}`, fontSize: 10, margin: [40, 5, 40, 5]},
          {
            columns: [
              {text: `RUN: ${run_paciente}      FECHA NACIMIENTO: ${fechaNacimientoParaPdf}      EDAD: ${edad} años`,  margin: [40, 0, 40, 0], fontSize: 10},
            ]
          },
          
          {text: `FECHA INGRESO: ${fecha_ingreso}    FECHA PROBABLE ALTA: ${fecha_alta}       SERVICIO: ${servicio}`, margin: [40, 10, 40, 5], fontSize: 10},
          {text: `__________________________________________________________________________________________________`,  alignment:'center'},
      ],


      content: [
        {text: 'DIAGNÓSTICO', margin: [ 0, 10, 0, 10 ], bold: true},
        {text: `${diagnostico}`, style: ['contenido'],margin: [ 0, 0, 0, 10 ], fontSize:10},
        {text: 'EVOLUCIÓN', bold: true, margin: [ 0, 10, 0, 10 ]},
        {text: `${evolucion}`, style: ['contenido'],margin: [ 0, 0, 0, 10 ], fontSize:10},
        {text: 'ESTUDIOS COMPLEMENTARIOS', bold: true, margin: [ 0, 10, 0, 10 ]},
        {text: `${estudios}`, style: ['contenido'],margin: [ 0, 0, 0, 10 ], fontSize:10},
        {text: 'PLAN', bold: true, margin: [ 0, 10, 0, 10 ]},
        {text: `${plan}`, style: ['contenido'],margin: [ 0, 0, 0, 10 ], fontSize:10},

      ],

      footer: [
        { text: `INTERNO MEDICINA: ${interno_medicina}`,style: ['footer'] ,margin: [ 40, 15, 5, 5 ]},
        {
          columns: [
            {text: `RUN MÉDICO: ${run_medico}       NOMBRE MÉDICO: ${nombreCompletoMedico}`, style: ['footer'] ,margin: [ 40, 0, 5, 0 ]}
          ]
        }
      ],
          // {text: `INTERNO MEDICINA: ${interno_medicina}RUN MÉDICO: ${run_medico}            NOMBRE MÉDICO: ${nombreCompletoMedico}`, style: ['footer'] ,margin: [ 40, 20, 20, 20 ]},

      styles: {
        titulo: {
          fontSize: 10,
          bold: true,
          alignment:'center'
        },
        cabecera: {
          fontSize: 10,
          bold: true,
          alignment:'left'
        },
        contenido: {
          italics: true,
          alignment: 'justify'
        },
        footer: {
          fontSize: 10,
          alignment: 'left',
        }
      }
      
    }

    const pdf = pdfMake.createPdf(documento_pdf);
    pdf.open();



  }


}
