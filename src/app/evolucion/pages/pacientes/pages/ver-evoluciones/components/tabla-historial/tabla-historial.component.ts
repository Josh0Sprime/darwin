...
export class TablaHistorialComponent {
  ...
  idPaciente: number;
  episodios:Episodio[] = [];
  evoluciones_episodio: Evolucion[] = [];
  detalle_evolucion: any[] = [];
  modalEvoluciones:boolean = false

  constructor( private evs: EvolucionService, private rutaActiva: ActivatedRoute){

    //obtener el id del paciente de la ruta
    const { id } = rutaActiva.snapshot.params;
    this.idPaciente = id;

    //obtener el listado de todos los episodios del paciente....
    this.evs.ObtenerEpisodiosPaciente(this.idPaciente).subscribe(response => {
      this.episodios = response;
    })
  }


  VerEvolucionesDelEpisodio = (Episodio:number) =>{
    this.modalEvoluciones = true;
    this.evoluciones_episodio = [];
    //obtener todas las evoluciones relacionadas al episodio seleccionado....
    this.evs.obtenerListadoDeEvolucionesPorEpisodio(Episodio).subscribe(response => {
      this.evoluciones_episodio = response;
    })
  }

  obtenerDetalleEvolucion = (idEvolucion:number) =>{

    this.evs.obtenerTodosLosDetallesEvolucionPDFActual(idEvolucion).subscribe(response => {
